...
SearchQuery query2() {
  ...
  query.set_return_limit(200);
  auto* fs = query.mutable_conditions()
                 ->mutable_sub_statement();  //->mutable_field_statement();
...
int main(int argc, char** argv) {
  ...
  std::unique_ptr<LogIndexer::Stub> stub =
      LogIndexer::NewStub(grpc::CreateChannel(
  ...
  std::cout << "Log searched: " << rst.total_count() << " ["
            << rst.search_result().searched_range().start().sec() << "-"
            << rst.search_result().searched_range().end().sec() << "]"
            << std::endl;
  std::cout << "Log returned: " << rst.search_result().log_size() << " ["
            << rst.search_result().returned_range().start().sec() << "-"
            << rst.search_result().returned_range().end().sec() << "]"
            << std::endl;
  ...
  for (auto& field_info : rst.field_infos()) {
    std::cout << "Field " << field_info.key() << " : "
              << field_info.total_count() << "::";
    if (field_info.values_size() > 0) {
      ...
      if (field_info.has_ranges()) {
        std::cout << "[" << field_info.ranges().min() << ", "
                  << field_info.ranges().max() << "]" << std::endl;
      }
    }
    std::cout << std::endl;
  }
  for (auto& ts : rst.search_result().time_series()) {
    std::cout << ts.name() << "::" << std::endl;
    for (auto& dis : ts.time_distribution()) {
