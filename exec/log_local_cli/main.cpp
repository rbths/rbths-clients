...
int main(int argc, char** argv) {
  ...
  auto creator = rbths::log_grabber::plugin_loader::getLogIteratorGenerator("journald");
  if (creator == nullptr) {
  ...
  std::cout << "Searched Range:" << searched_range.start << " -> " << searched_range.end << " : " << (searched_range.end - searched_range.start) << " returned " << results.size() << std::endl;
  ...
  for(auto& key: keys) {
    std::cout << "String field " << key << " : " << groups[key][LOG_GRABBER_TOTAL_COUNT_KEY] << std::endl;
  }

  for(auto& key: ranges) {
    std::cout << "Int field " << key.first << " : [" << key.second.first << "," << key.second.second << "]: " << groups[key.first][LOG_GRABBER_TOTAL_COUNT_KEY] << std::endl;
  }
