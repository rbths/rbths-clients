...
bool check_condition(const ConditionStatement& condition, const LogEntry& entry) {
    if (condition.sub_condition.has_value()) {
        const auto& sub_condition = condition.sub_condition.value();
        switch(sub_condition.sub_op) {
            case SubOps::AND:
                for (auto& sub_condition : condition.sub_condition.value().sub_conditions) {
    ...
    if (condition.field_statement.has_value()) {
            ...
            for (auto& field : entry.additional_fields) {
                if (field.key == field_statement.key) {
                    if(field.key.find("TIMESTAMP") != std::string::npos) {
                        return int64_t(std::stod(field.value) * 1000000) > field_statement.greater.value();
                    }
                }
            }
            return false;
        ...
        if (field_statement.values_in.has_value()) {
            ...
            if (field_statement.key == "msg") {
                return std::find(values.begin(), values.end(), entry.msg) != values.end();
            }
            if (field_statement.key == "service") {
                return std::find(values.begin(), values.end(), entry.service) != values.end();
            }
            if (field_statement.key == "hostname") {
                return std::find(values.begin(), values.end(), entry.hostname) != values.end();
            }
            if (field_statement.key == "unit") {
                return std::find(values.begin(), values.end(), entry.unit) != values.end();
            }
            for (auto& field : entry.additional_fields) {
                if (field.key == field_statement.key) {
                    return std::find(values.begin(), values.end(), field.value) != values.end();
                }
            }
            return false;
        }
...
void IteratorGrabber::searchLog(const LogSearchInput& input, LogRange& searched_range, LogRange& returned_range, std::vector<LogEntry> &results) {
    ...
    while(auto entry = iterator->next()) {
        if (input.conditions.has_value() && !check_condition(input.conditions.value(), entry.value())) {
            continue;
        }
...
inline void update_key_value_range_int(const std::string& key, const int64_t value,  std::unordered_map<std::string, std::unordered_map<std::string, int>> &groups, std::unordered_map<std::string, std::pair<int64_t, int64_t>> &ranges, const int limit = 30) {
...
inline void update_key_value_range_str(const std::string& key, const std::string& value,  std::unordered_map<std::string, std::unordered_map<std::string, int>> &groups, std::unordered_set<std::string> &keys, const int limit=100) {
...
void IteratorGrabber::searchAndGroup(const LogSearchInput& input, LogRange& searched_range, LogRange& returned_range, std::vector<LogEntry> &results, std::unordered_map<std::string, std::unordered_map<std::string, int>> &groups, std::unordered_map<std::string, std::pair<int64_t, int64_t>> &ranges, std::unordered_set<std::string> &keys) {
    uint64_t t0 = getTime_us();
    int64_t return_limit = input.return_limit;
    auto iterator = iterator_generator(input);
    while(auto entry_p = iterator->next()) {
        ...
        if (input.conditions.has_value() && !check_condition(input.conditions.value(), entry_p.value())) {
            if (no_more) {
                break;
            }
            continue;
